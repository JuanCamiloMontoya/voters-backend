...
export class AuthService {
  ...
  async passwordResetRequest(body: PasswordResetRequestDTO) {
    try {
      ...
      await this.sengridService.sendEmail(email, Template.RECOVER_PASSWORD, {
        passwordResetCode,
      });
  ...
  async verifyEmail(body: VerifyEmailDTO) {
    try {
      ...
      const isMatch =
        user && (await bcrypt.compare(code, user.passwordResetCode));
      ...
      let passwordResetCode = Math.floor(
        100000 + Math.random() * 900000,
      ).toString();
      const encrytedCode = await bcrypt.hash(passwordResetCode, 10);
      ...
      await this.userRepository.update(
        { email },
        { passwordResetCode: encrytedCode },
      );
